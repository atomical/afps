...
namespace {
using afps::server::ResolveDetailedWorldRaycast;
using afps::server::ResolveWorldHitBackendMode;
using afps::server::ResolveWorldRaycast;
using afps::server::ToSurfaceType;
using afps::server::WorldHitBackendMode;
using afps::server::WorldHitBackendModeName;
using afps::server::WorldHitBackendName;
using afps::server::WorldHitResult;
...
int16_t QuantizeI16(double value, double step) {
  ...
  const int64_t clamped =
      std::max<int64_t>(std::numeric_limits<int16_t>::min(),
...
void WriteWorldHitJson(std::ostream &out, const WorldHitResult &hit) {
  out << "{\"hit\":" << (hit.hit ? "true" : "false")
      << ",\"distance\":" << (std::isfinite(hit.distance) ? hit.distance : -1.0)
      << ",\"collider_id\":" << hit.collider_id << ",\"backend\":\""
      << WorldHitBackendName(hit.backend) << "\""
      << ",\"instance_id\":" << hit.instance_id << ",\"prefab_id\":\""
      << EscapeJsonText(hit.prefab_id) << "\""
      << ",\"face_id\":" << hit.face_id << ",\"surface\":\"" << SurfaceTypeName(hit.surface) << "\""
      << ",\"position\":";
...
void LogHitscanShotDebug(int server_tick, const std::string &shooter_id,
                         const std::string &weapon_id, int weapon_slot, int shot_seq,
                         int estimated_tick, const FireWeaponRequest &request,
                         const afps::combat::Vec3 &origin, const afps::combat::Vec3 &muzzle,
                         const afps::combat::Vec3 &dir, double max_range, double intended_distance,
                         const afps::combat::HitResult &player_hit,
                         const WorldHitResult &eye_world_hit, bool muzzle_block_checked,
                         const WorldHitResult &muzzle_block_hit, bool retry_attempted,
                         bool retry_suppressed, bool retry_hit,
                         const WorldHitResult &retry_world_hit, const char *world_hit_source,
                         WorldHitBackendMode world_hit_backend_mode,
                         const WorldHitResult &final_world_hit, bool shadow_world_checked,
                         const ShadowDetailedWorldHit &shadow_world_hit, HitKind final_hit_kind,
                         const std::string &final_hit_target, double final_hit_distance,
                         const afps::combat::Vec3 &final_hit_position,
                         const afps::combat::Vec3 &final_hit_normal, SurfaceType final_surface) {
  ...
  out << "{\"event\":\"shot_debug\""
      << ",\"server_tick\":" << server_tick << ",\"shooter_id\":\"" << EscapeJsonText(shooter_id)
      << "\""
      << ",\"shot_seq\":" << shot_seq << ",\"client_shot_seq\":" << request.client_shot_seq
      << ",\"estimated_tick\":" << estimated_tick << ",\"weapon_slot\":" << weapon_slot
      ...
      << ",\"y\":" << request.debug_player_pos_y << ",\"z\":" << request.debug_player_pos_z << "}"
  ...
  out << ",\"player_hit\":{\"hit\":" << (player_hit.hit ? "true" : "false") << ",\"target_id\":\""
      << EscapeJsonText(player_hit.target_id) << "\""
      << ",\"distance\":" << (std::isfinite(player_hit.distance) ? player_hit.distance : -1.0)
      << ",\"position\":";
  WriteVec3Json(out, player_hit.position);
  out << "}"
      << ",\"world_hit_backend_mode\":\"" << WorldHitBackendModeName(world_hit_backend_mode) << "\""
  ...
  out << ",\"world_retry_attempted\":" << (retry_attempted ? "true" : "false")
      ...
      << ",\"world_retry_hit\":" << (retry_hit ? "true" : "false") << ",\"world_retry\":";
...
void LogClientDecalDebug(int server_tick, const std::string &connection_id, const InputCmd &cmd,
                         const afps::sim::PlayerState *player_state) {
  if (!cmd.debug_decal_report_present) {
    return;
  }

  std::ostringstream out;
  out << "{\"event\":\"client_decal_debug\""
      << ",\"server_tick\":" << server_tick << ",\"connection_id\":\""
      << EscapeJsonText(connection_id) << "\""
      << ",\"input_seq\":" << cmd.input_seq
      << ",\"client_report\":{\"server_tick\":" << cmd.debug_decal_server_tick
      << ",\"shot_seq\":" << cmd.debug_decal_shot_seq
      << ",\"hit_kind\":" << static_cast<int>(cmd.debug_decal_hit_kind)
      << ",\"surface_type\":" << static_cast<int>(cmd.debug_decal_surface_type)
      << ",\"authoritative_world_hit\":"
      << (cmd.debug_decal_authoritative_world_hit ? "true" : "false")
      << ",\"used_projected_hit\":" << (cmd.debug_decal_used_projected_hit ? "true" : "false")
      << ",\"used_impact_projection\":"
      << (cmd.debug_decal_used_impact_projection ? "true" : "false")
      << ",\"decal_spawned\":" << (cmd.debug_decal_spawned ? "true" : "false")
      << ",\"decal_in_frustum\":" << (cmd.debug_decal_in_frustum ? "true" : "false")
      << ",\"decal_distance\":" << cmd.debug_decal_distance
      << ",\"decal_position\":{\"x\":" << cmd.debug_decal_position_x
      << ",\"y\":" << cmd.debug_decal_position_y << ",\"z\":" << cmd.debug_decal_position_z << "}"
      << ",\"decal_normal\":{\"x\":" << cmd.debug_decal_normal_x
      << ",\"y\":" << cmd.debug_decal_normal_y << ",\"z\":" << cmd.debug_decal_normal_z << "}"
      << ",\"trace_hit_position\":{\"x\":" << cmd.debug_trace_hit_position_x
      << ",\"y\":" << cmd.debug_trace_hit_position_y << ",\"z\":" << cmd.debug_trace_hit_position_z
      << "}"
      << ",\"trace_hit_normal\":{\"x\":" << cmd.debug_trace_hit_normal_x
      << ",\"y\":" << cmd.debug_trace_hit_normal_y << ",\"z\":" << cmd.debug_trace_hit_normal_z
      << "}"
      << "}"
      << ",\"server_view\":{\"yaw\":" << cmd.view_yaw << ",\"pitch\":" << cmd.view_pitch << "}";
  if (player_state) {
    out << ",\"server_player\":{\"x\":" << player_state->x << ",\"y\":" << player_state->y
        << ",\"z\":" << player_state->z << "}";
  } else {
    out << ",\"server_player\":null";
  }
  out << "}";
  WriteShotDebugLine(out.str());
}
...
ShadowDetailedWorldHit
ResolveShadowDetailedWorldHitscan(const afps::combat::Vec3 &origin, const afps::combat::Vec3 &dir,
                                  double max_distance,
  ...
  ShadowDetailedWorldHit out;
  const WorldHitResult hit =
      ResolveDetailedWorldRaycast(origin, dir, max_distance, instances, registry, prefab_lookup);
  if (!hit.hit) {
    return out;
  }
  out.hit = true;
  ...
  out.prefab_id = hit.prefab_id;
  out.instance_id = hit.instance_id;
  out.face_id = hit.face_id;
  return out;
}
...
afps::combat::Vec3 ApplySpread(const afps::combat::Vec3 &dir, double spread_deg, uint32_t seed) {
  ...
  const afps::combat::Vec3 spread_dir =
      Add(Add(Mul(forward, cos_theta), Mul(right, sin_theta * std::cos(phi))),
...
double SegmentSegmentDistanceSquared(const afps::combat::Vec3 &p1, const afps::combat::Vec3 &q1,
                                     const afps::combat::Vec3 &p2, const afps::combat::Vec3 &q2) {
...
bool IsSpawnPointBlocked(const afps::sim::CollisionWorld &world, const afps::sim::SimConfig &config,
...
bool ResolveSpawnPoint(const afps::sim::CollisionWorld &world, const afps::sim::SimConfig &config,
                       double start_angle, double &out_x, double &out_y) {
  const double half = (std::isfinite(config.arena_half_size) && config.arena_half_size > 0.0)
  ...
  for (double ring : ring_radii) {
    for (int step = 0; step < kSpawnAngleSamples; ++step) {
      const double angle = start_angle + (span * static_cast<double>(step) /
                                          static_cast<double>(kSpawnAngleSamples));
...
bool ResolveRandomSpawnPoint(const afps::sim::CollisionWorld &world,
                             const afps::sim::SimConfig &config, std::mt19937 &rng, double &out_x,
                             double &out_y) {
  const double half = (std::isfinite(config.arena_half_size) && config.arena_half_size > 0.0)
                          ? config.arena_half_size
                          : 10.0;
...
void LogSpawnState(const std::string &connection_id, const afps::sim::PlayerState &state,
                   const char *reason) {
...
TickLoop::TickLoop(SignalingStore &store, int tick_rate, int snapshot_keyframe_interval,
                   uint32_t map_seed, const afps::world::MapWorldOptions &map_options)
    : store_(store), accumulator_(tick_rate),
      snapshot_keyframe_interval_(snapshot_keyframe_interval), map_seed_(map_seed),
      map_options_(map_options) {
  pose_history_limit_ = std::max(1, accumulator_.tick_rate() * 2);
  std::string weapon_error;
  weapon_config_ =
      afps::weapons::LoadWeaponConfig(afps::weapons::ResolveWeaponConfigPath(), weapon_error);
  if (!weapon_error.empty()) {
    std::cerr << "[warn] " << weapon_error << "\n";
  }
  const auto generated =
      afps::world::GenerateMapWorld(sim_config_, map_seed_, accumulator_.tick_rate(), map_options_);
  ...
  for (const auto &instance : static_mesh_instances_) {
    ...
    for (int collider_id = instance.first_collider_id; collider_id <= instance.last_collider_id;
         ++collider_id) {
  ...
  std::cout << "{\"event\":\"world_hit_backend_mode\",\"mode\":\""
            << WorldHitBackendModeName(ResolveWorldHitBackendMode())
            << "\",\"collision_mesh_enabled\":"
            << ((collision_mesh_registry_loaded_ && !static_mesh_instances_.empty()) ? "true"
                                                                                     : "false")
            << "}\n";
  pickups_.clear();
  ...
  while (running_.load()) {
    ...
    if (now - last_log_time_ >= std::chrono::seconds(1)) {
      ...
      std::cout << "[tick] rate=" << accumulator_.tick_rate() << " ticks=" << tick_count_
                << " conns=" << connections << " batches=" << batch_count_
                << " inputs=" << input_count_ << " snapshots=" << snapshot_count_ << "\n";
      tick_count_ = 0;
      batch_count_ = 0;
      input_count_ = 0;
      snapshot_count_ = 0;
...
void TickLoop::Step() {
  ...
  auto to_spawn_fx = [](const TickLoop::PickupState &pickup) {
    ...
    fx.kind = pickup.definition.kind == afps::world::PickupKind::Weapon ? PickupKind::Weapon
                                                                        : PickupKind::Health;
  ...
  for (const auto &connection_id : active_ids) {
    ...
    if (!active_pickups.empty()) {
      ...
      while (index < active_pickups.size()) {
        ...
        const auto payload = BuildGameEventBatch(batch, store_.NextServerMessageSeq(connection_id),
  ...
  auto resolve_view = [&](const std::string &connection_id) {
    ...
    return afps::combat::SanitizeViewAngles(input_iter->second.view_yaw,
                                            input_iter->second.view_pitch);
  };
  auto resolve_fire_view = [&](const std::string &connection_id, const FireWeaponRequest &request) {
    ...
    const double request_len_sq = request_dir.x * request_dir.x + request_dir.y * request_dir.y +
                                  request_dir.z * request_dir.z;
  ...
  auto resolve_max_ammo = [&](const afps::weapons::WeaponDef *weapon,
                              uint32_t loadout_bits) -> int {
    if (!weapon) {
      return 0;
    }
  ...
  auto resolve_reload_seconds = [&](const afps::weapons::WeaponDef *weapon,
  ...
  auto resolve_spread_deg = [&](const afps::weapons::WeaponDef *weapon,
                                const WeaponSlotState &slot_state, const InputCmd &input,
  ...
  for (const auto &batch : batches) {
    ...
    for (const auto &cmd : batch.inputs) {
      ...
      const auto player_iter = players_.find(batch.connection_id);
      const afps::sim::PlayerState *player_state =
          (player_iter == players_.end()) ? nullptr : &player_iter->second;
      LogClientDecalDebug(server_tick_, batch.connection_id, cmd, player_state);
    }
    if (max_seq >= 0) {
  ...
  for (const auto &connection_id : active_ids) {
    ...
    if (combat_state.alive) {
      const auto sim_input = afps::sim::MakeInput(
          input.move_x, input.move_y, input.sprint, input.jump, input.dash, input.grapple,
  ...
  const double player_height =
      (std::isfinite(sim_config_.player_height) && sim_config_.player_height > 0.0)
          ? sim_config_.player_height
          : 1.7;
  for (auto &pickup : pickups_) {
    ...
    for (const auto &connection_id : active_ids) {
      ...
      if (pickup.definition.position.z < player_min_z - 0.5 ||
          pickup.definition.position.z > player_max_z + 0.5) {
        continue;
      }
    ...
    if (pickup.definition.kind == afps::world::PickupKind::Health) {
      ...
      if (combat_iter != combat_states_.end()) {
        const double amount =
            pickup.definition.amount > 0 ? static_cast<double>(pickup.definition.amount) : 25.0;
        combat_iter->second.health =
            std::min(afps::combat::kMaxHealth, combat_iter->second.health + amount);
      }
      ...
      if (weapon_iter != weapon_states_.end() && !weapon_iter->second.slots.empty()) {
        ...
        if (max_ammo > 0) {
          ...
          if (pickup.definition.amount > 0) {
            slot_state.ammo_in_mag =
                std::min(max_ammo, slot_state.ammo_in_mag + pickup.definition.amount);
          } else {
  ...
  for (auto &entry : weapon_states_) {
    ...
    for (size_t i = 0; i < state.slots.size(); ++i) {
      ...
      if (slot_state.reload_timer > 0.0) {
        ...
        if (slot_state.reload_timer <= 0.0) {
          const auto *weapon =
              afps::weapons::ResolveWeaponSlot(weapon_config_, static_cast<int>(i));
    ...
    for (const auto &event : shockwave_events) {
      ...
      for (const auto &hit : hits) {
        ...
        if (hit.damage > 0.0) {
          ...
          killed = afps::combat::ApplyDamageWithShield(target_combat_iter->second, attacker,
                                                       hit.damage, shield_active && shield_facing,
  ...
  auto should_show_tracer = [&](const afps::weapons::WeaponDef *weapon, int shot_seq,
  ...
  for (const auto &event : fire_events) {
    ...
    const double spread_deg =
        resolve_spread_deg(weapon, slot_state, input, state_iter->second, loadout_bits);
    const uint32_t spread_seed = HashString(event.connection_id) ^
    ...
    if (weapon->kind == afps::weapons::WeaponKind::kHitscan) {
      ...
      const auto result =
          afps::combat::ResolveHitscan(event.connection_id, pose_histories_, estimated_tick,
                                       shot_view, sim_config_, weapon->range, nullptr);
      const WorldHitBackendMode world_hit_backend_mode = ResolveWorldHitBackendMode();
      const bool collision_mesh_enabled =
          collision_mesh_registry_loaded_ && !static_mesh_instances_.empty();
      WorldHitResult world_hit = ResolveWorldRaycast(
          origin, shot_dir, sim_config_, &collision_world_, static_mesh_instances_,
          collision_mesh_registry_, collision_mesh_prefab_lookup_, collision_mesh_enabled,
          weapon->range, world_hit_backend_mode, &collider_instance_lookup_);
      const WorldHitResult eye_world_hit = world_hit;
      WorldHitResult muzzle_block_hit;
      bool muzzle_block_checked = false;
      bool retry_attempted = false;
      bool retry_suppressed = false;
      bool retry_hit = false;
      WorldHitResult retry_world_hit;
      std::string world_hit_source = world_hit.hit ? "eye" : "none";
      const double intended_distance = world_hit.hit ? world_hit.distance : max_range;
      if (std::isfinite(intended_distance) && intended_distance > 0.0) {
        ...
        const auto muzzle_block = ResolveWorldRaycast(
            muzzle, shot_dir, sim_config_, &collision_world_, static_mesh_instances_,
            collision_mesh_registry_, collision_mesh_prefab_lookup_, collision_mesh_enabled,
            intended_distance, world_hit_backend_mode, &collider_instance_lookup_,
            muzzle_trace_options);
        if (muzzle_block.hit) {
        ...
        if (muzzle_block.hit &&
          ...
          const bool near_muzzle_block = muzzle_block.distance <= kShotNearMuzzleGraceMeters;
          ...
          if (near_muzzle_block && can_retry_ignore) {
            ...
            const auto retrace_hit = ResolveWorldRaycast(
                muzzle, shot_dir, sim_config_, &collision_world_, static_mesh_instances_,
                collision_mesh_registry_, collision_mesh_prefab_lookup_, collision_mesh_enabled,
                intended_distance, world_hit_backend_mode, &collider_instance_lookup_,
                retry_options, retry_ignore_instance_id);
            if (retrace_hit.hit) {
              retry_hit = true;
            ...
            if (!retrace_hit.hit ||
              ...
              world_hit.distance = std::min(
                  max_range, std::max(0.0, retrace_hit.distance + kShotMuzzleOffsetMeters));
              world_hit.position = Add(origin, Mul(shot_dir, world_hit.distance));
              world_hit_source = "muzzle_retry";
            }
          }
          if (!suppressed_near_muzzle_block && !(near_muzzle_block && can_retry_ignore)) {
            world_hit = muzzle_block;
            world_hit.distance =
                std::min(max_range, std::max(0.0, muzzle_block.distance + kShotMuzzleOffsetMeters));
            world_hit.position = Add(origin, Mul(shot_dir, world_hit.distance));
      ...
      if (world_hit.hit && world_hit.backend == WorldHitResult::Backend::Aabb &&
          world_hit.collider_id == -1 && std::abs(world_hit.normal.z) < 0.5) {
        world_hit.hit = false;
        world_hit.distance = max_range;
        world_hit.backend = WorldHitResult::Backend::None;
        world_hit.instance_id = 0;
        world_hit.face_id = -1;
        world_hit.prefab_id.clear();
        world_hit_source = "arena_side_ignored";
      }
      if (world_hit.hit && world_hit.backend == WorldHitResult::Backend::MeshBvh &&
      ...
      const afps::combat::Vec3 hit_position = (hit_kind == HitKind::World)
                                                  ? world_hit.position
                                                  : Add(origin, Mul(shot_dir, hit_distance));
      ...
      if (event.request.debug_enabled || ShouldLogShotDebug()) {
        ...
        if (shadow_world_checked) {
          shadow_world_hit = ResolveShadowDetailedWorldHitscan(
              origin, shot_dir, hit_distance, static_mesh_instances_, collision_mesh_registry_,
              collision_mesh_prefab_lookup_);
        }
      }
      LogHitscanShotDebug(server_tick_, event.connection_id, weapon->id, active_slot, shot_seq,
                          estimated_tick, event.request, origin, muzzle, shot_dir, max_range,
                          intended_distance, result, eye_world_hit, muzzle_block_checked,
                          muzzle_block_hit, retry_attempted, retry_suppressed, retry_hit,
                          retry_world_hit, world_hit_source.c_str(), world_hit_backend_mode,
                          world_hit, shadow_world_checked, shadow_world_hit, hit_kind, hit_target,
                          hit_distance, hit_position, hit_normal, surface_type);
      ...
      if (hit_kind == HitKind::Player) {
        ...
        if (target_iter != combat_states_.end()) {
          ...
          killed = afps::combat::ApplyDamageWithShield(
              target_iter->second, &shooter_iter->second, weapon->damage,
              shield_active && shield_facing, sim_config_.shield_damage_multiplier);
          if (killed) {
      ...
      const double capsule_radius =
          (std::isfinite(sim_config_.player_radius) && sim_config_.player_radius > 0.0)
      ...
      if (weapon->projectile_speed > 0.0 && std::isfinite(weapon->projectile_speed)) {
        ...
        projectile.velocity = {shot_dir.x * weapon->projectile_speed,
                               shot_dir.y * weapon->projectile_speed,
  ...
  if (!projectiles_.empty()) {
    ...
    const WorldHitBackendMode projectile_world_backend_mode = ResolveWorldHitBackendMode();
    const bool projectile_collision_mesh_enabled =
        collision_mesh_registry_loaded_ && !static_mesh_instances_.empty();
    const afps::combat::ProjectileWorldImpactResolver projectile_world_resolver =
        [&](const afps::combat::ProjectileState &projectile_state,
            const afps::combat::Vec3 &projectile_delta, const afps::sim::SimConfig &config,
            const afps::sim::CollisionWorld *world) {
          afps::combat::ProjectileWorldImpact out;
          const double len_sq = Dot(projectile_delta, projectile_delta);
          const double len = std::sqrt(len_sq);
          if (!std::isfinite(len) || len <= 1e-9) {
            return out;
          }
          const afps::combat::Vec3 dir{projectile_delta.x / len, projectile_delta.y / len,
                                       projectile_delta.z / len};
          const WorldHitResult hit =
              ResolveWorldRaycast(projectile_state.position, dir, config, world,
                                  static_mesh_instances_, collision_mesh_registry_,
                                  collision_mesh_prefab_lookup_, projectile_collision_mesh_enabled,
                                  len, projectile_world_backend_mode, &collider_instance_lookup_);
          if (!hit.hit || !std::isfinite(hit.distance) || hit.distance < 0.0 ||
              hit.distance > len) {
            return out;
          }
          out.hit = true;
    ...
    for (auto &projectile : projectiles_) {
      ...
      if (impact.hit) {
        ...
        for (const auto &hit : hits) {
          ...
          const bool killed = afps::combat::ApplyDamageWithShield(
              target_iter->second, attacker, hit.damage, shield_active && shield_facing,
              sim_config_.shield_damage_multiplier);
          HitConfirmedFx hit_event;
          hit_event.target_id = hit.target_id;
          hit_event.damage = hit.damage;
        ...
        SurfaceType surface =
            impact.hit_world ? ToSurfaceType(impact.surface_type) : SurfaceType::Energy;
  ...
  for (const auto &recipient_id : active_ids) {
    ...
    while (!events.empty()) {
      ...
      if (probe.size() <= kMaxClientMessageBytes) {
        const auto payload =
            BuildGameEventBatch(batch, store_.NextServerMessageSeq(recipient_id), server_seq_ack);
  ...
  for (const auto &recipient_id : active_ids) {
    ...
    while (index < events.size()) {
      ...
      while (count > 0 && !sent) {
        GameEventBatch batch;
        batch.server_tick = server_tick_;
        batch.events.insert(batch.events.end(), events.begin() + static_cast<long>(index),
                            events.begin() + static_cast<long>(index + count));
        const auto payload = BuildGameEventBatch(batch, store_.NextServerMessageSeq(recipient_id),
  ...
  if (accumulator_.tick_rate() > 0) {
    snapshot_accumulator_ +=
        static_cast<double>(kSnapshotRate) / static_cast<double>(accumulator_.tick_rate());
  }
  if (snapshot_accumulator_ >= 1.0) {
    ...
    for (const auto &connection_id : active_ids) {
      ...
      snapshot.last_processed_input_seq =
          (seq_iter == last_input_seq_.end()) ? -1 : seq_iter->second;
      auto input_iter = last_inputs_.find(connection_id);
      snapshot.weapon_slot =
          (input_iter == last_inputs_.end()) ? 0 : input_iter->second.weapon_slot;
      if (!weapon_config_.slots.empty()) {
        const int max_slot = static_cast<int>(weapon_config_.slots.size() - 1);
        snapshot.weapon_slot = std::min(snapshot.weapon_slot, max_slot);
      }
      auto weapon_state_iter = weapon_states_.find(connection_id);
      if (weapon_state_iter != weapon_states_.end() && snapshot.weapon_slot >= 0 &&
      ...
      if (needs_full) {
        for (const auto &recipient_id : active_ids) {
          const auto payload =
              BuildStateSnapshot(snapshot, store_.NextServerMessageSeq(recipient_id),
                                 store_.LastClientMessageSeq(recipient_id));
      ...
      for (const auto &recipient_id : active_ids) {
        const auto payload =
            BuildStateSnapshotDelta(delta, store_.NextServerMessageSeq(recipient_id),
                                    store_.LastClientMessageSeq(recipient_id));
        if (store_.SendUnreliable(recipient_id, payload)) {
          snapshot_count_ += 1;
        }
