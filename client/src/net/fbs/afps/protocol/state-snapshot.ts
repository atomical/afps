// automatically generated by the FlatBuffers compiler, do not modify

/* eslint-disable @typescript-eslint/no-unused-vars, @typescript-eslint/no-explicit-any, @typescript-eslint/no-non-null-assertion */

import * as flatbuffers from 'flatbuffers';



export class StateSnapshot implements flatbuffers.IUnpackableObject<StateSnapshotT> {
  bb: flatbuffers.ByteBuffer|null = null;
  bb_pos = 0;
  __init(i:number, bb:flatbuffers.ByteBuffer):StateSnapshot {
  this.bb_pos = i;
  this.bb = bb;
  return this;
}

static getRootAsStateSnapshot(bb:flatbuffers.ByteBuffer, obj?:StateSnapshot):StateSnapshot {
  return (obj || new StateSnapshot()).__init(bb.readInt32(bb.position()) + bb.position(), bb);
}

static getSizePrefixedRootAsStateSnapshot(bb:flatbuffers.ByteBuffer, obj?:StateSnapshot):StateSnapshot {
  bb.setPosition(bb.position() + flatbuffers.SIZE_PREFIX_LENGTH);
  return (obj || new StateSnapshot()).__init(bb.readInt32(bb.position()) + bb.position(), bb);
}

serverTick():number {
  const offset = this.bb!.__offset(this.bb_pos, 4);
  return offset ? this.bb!.readInt32(this.bb_pos + offset) : 0;
}

lastProcessedInputSeq():number {
  const offset = this.bb!.__offset(this.bb_pos, 6);
  return offset ? this.bb!.readInt32(this.bb_pos + offset) : 0;
}

clientId():string|null
clientId(optionalEncoding:flatbuffers.Encoding):string|Uint8Array|null
clientId(optionalEncoding?:any):string|Uint8Array|null {
  const offset = this.bb!.__offset(this.bb_pos, 8);
  return offset ? this.bb!.__string(this.bb_pos + offset, optionalEncoding) : null;
}

posX():number {
  const offset = this.bb!.__offset(this.bb_pos, 10);
  return offset ? this.bb!.readFloat64(this.bb_pos + offset) : 0.0;
}

posY():number {
  const offset = this.bb!.__offset(this.bb_pos, 12);
  return offset ? this.bb!.readFloat64(this.bb_pos + offset) : 0.0;
}

posZ():number {
  const offset = this.bb!.__offset(this.bb_pos, 14);
  return offset ? this.bb!.readFloat64(this.bb_pos + offset) : 0.0;
}

velX():number {
  const offset = this.bb!.__offset(this.bb_pos, 16);
  return offset ? this.bb!.readFloat64(this.bb_pos + offset) : 0.0;
}

velY():number {
  const offset = this.bb!.__offset(this.bb_pos, 18);
  return offset ? this.bb!.readFloat64(this.bb_pos + offset) : 0.0;
}

velZ():number {
  const offset = this.bb!.__offset(this.bb_pos, 20);
  return offset ? this.bb!.readFloat64(this.bb_pos + offset) : 0.0;
}

weaponSlot():number {
  const offset = this.bb!.__offset(this.bb_pos, 22);
  return offset ? this.bb!.readInt32(this.bb_pos + offset) : 0;
}

ammoInMag():number {
  const offset = this.bb!.__offset(this.bb_pos, 24);
  return offset ? this.bb!.readInt32(this.bb_pos + offset) : 0;
}

dashCooldown():number {
  const offset = this.bb!.__offset(this.bb_pos, 26);
  return offset ? this.bb!.readFloat64(this.bb_pos + offset) : 0.0;
}

health():number {
  const offset = this.bb!.__offset(this.bb_pos, 28);
  return offset ? this.bb!.readFloat64(this.bb_pos + offset) : 0.0;
}

kills():number {
  const offset = this.bb!.__offset(this.bb_pos, 30);
  return offset ? this.bb!.readInt32(this.bb_pos + offset) : 0;
}

deaths():number {
  const offset = this.bb!.__offset(this.bb_pos, 32);
  return offset ? this.bb!.readInt32(this.bb_pos + offset) : 0;
}

viewYawQ():number {
  const offset = this.bb!.__offset(this.bb_pos, 34);
  return offset ? this.bb!.readInt16(this.bb_pos + offset) : 0;
}

viewPitchQ():number {
  const offset = this.bb!.__offset(this.bb_pos, 36);
  return offset ? this.bb!.readInt16(this.bb_pos + offset) : 0;
}

playerFlags():number {
  const offset = this.bb!.__offset(this.bb_pos, 38);
  return offset ? this.bb!.readUint8(this.bb_pos + offset) : 0;
}

weaponHeatQ():number {
  const offset = this.bb!.__offset(this.bb_pos, 40);
  return offset ? this.bb!.readUint16(this.bb_pos + offset) : 0;
}

loadoutBits():number {
  const offset = this.bb!.__offset(this.bb_pos, 42);
  return offset ? this.bb!.readUint32(this.bb_pos + offset) : 0;
}

static startStateSnapshot(builder:flatbuffers.Builder) {
  builder.startObject(20);
}

static addServerTick(builder:flatbuffers.Builder, serverTick:number) {
  builder.addFieldInt32(0, serverTick, 0);
}

static addLastProcessedInputSeq(builder:flatbuffers.Builder, lastProcessedInputSeq:number) {
  builder.addFieldInt32(1, lastProcessedInputSeq, 0);
}

static addClientId(builder:flatbuffers.Builder, clientIdOffset:flatbuffers.Offset) {
  builder.addFieldOffset(2, clientIdOffset, 0);
}

static addPosX(builder:flatbuffers.Builder, posX:number) {
  builder.addFieldFloat64(3, posX, 0.0);
}

static addPosY(builder:flatbuffers.Builder, posY:number) {
  builder.addFieldFloat64(4, posY, 0.0);
}

static addPosZ(builder:flatbuffers.Builder, posZ:number) {
  builder.addFieldFloat64(5, posZ, 0.0);
}

static addVelX(builder:flatbuffers.Builder, velX:number) {
  builder.addFieldFloat64(6, velX, 0.0);
}

static addVelY(builder:flatbuffers.Builder, velY:number) {
  builder.addFieldFloat64(7, velY, 0.0);
}

static addVelZ(builder:flatbuffers.Builder, velZ:number) {
  builder.addFieldFloat64(8, velZ, 0.0);
}

static addWeaponSlot(builder:flatbuffers.Builder, weaponSlot:number) {
  builder.addFieldInt32(9, weaponSlot, 0);
}

static addAmmoInMag(builder:flatbuffers.Builder, ammoInMag:number) {
  builder.addFieldInt32(10, ammoInMag, 0);
}

static addDashCooldown(builder:flatbuffers.Builder, dashCooldown:number) {
  builder.addFieldFloat64(11, dashCooldown, 0.0);
}

static addHealth(builder:flatbuffers.Builder, health:number) {
  builder.addFieldFloat64(12, health, 0.0);
}

static addKills(builder:flatbuffers.Builder, kills:number) {
  builder.addFieldInt32(13, kills, 0);
}

static addDeaths(builder:flatbuffers.Builder, deaths:number) {
  builder.addFieldInt32(14, deaths, 0);
}

static addViewYawQ(builder:flatbuffers.Builder, viewYawQ:number) {
  builder.addFieldInt16(15, viewYawQ, 0);
}

static addViewPitchQ(builder:flatbuffers.Builder, viewPitchQ:number) {
  builder.addFieldInt16(16, viewPitchQ, 0);
}

static addPlayerFlags(builder:flatbuffers.Builder, playerFlags:number) {
  builder.addFieldInt8(17, playerFlags, 0);
}

static addWeaponHeatQ(builder:flatbuffers.Builder, weaponHeatQ:number) {
  builder.addFieldInt16(18, weaponHeatQ, 0);
}

static addLoadoutBits(builder:flatbuffers.Builder, loadoutBits:number) {
  builder.addFieldInt32(19, loadoutBits, 0);
}

static endStateSnapshot(builder:flatbuffers.Builder):flatbuffers.Offset {
  const offset = builder.endObject();
  return offset;
}

static createStateSnapshot(builder:flatbuffers.Builder, serverTick:number, lastProcessedInputSeq:number, clientIdOffset:flatbuffers.Offset, posX:number, posY:number, posZ:number, velX:number, velY:number, velZ:number, weaponSlot:number, ammoInMag:number, dashCooldown:number, health:number, kills:number, deaths:number, viewYawQ:number, viewPitchQ:number, playerFlags:number, weaponHeatQ:number, loadoutBits:number):flatbuffers.Offset {
  StateSnapshot.startStateSnapshot(builder);
  StateSnapshot.addServerTick(builder, serverTick);
  StateSnapshot.addLastProcessedInputSeq(builder, lastProcessedInputSeq);
  StateSnapshot.addClientId(builder, clientIdOffset);
  StateSnapshot.addPosX(builder, posX);
  StateSnapshot.addPosY(builder, posY);
  StateSnapshot.addPosZ(builder, posZ);
  StateSnapshot.addVelX(builder, velX);
  StateSnapshot.addVelY(builder, velY);
  StateSnapshot.addVelZ(builder, velZ);
  StateSnapshot.addWeaponSlot(builder, weaponSlot);
  StateSnapshot.addAmmoInMag(builder, ammoInMag);
  StateSnapshot.addDashCooldown(builder, dashCooldown);
  StateSnapshot.addHealth(builder, health);
  StateSnapshot.addKills(builder, kills);
  StateSnapshot.addDeaths(builder, deaths);
  StateSnapshot.addViewYawQ(builder, viewYawQ);
  StateSnapshot.addViewPitchQ(builder, viewPitchQ);
  StateSnapshot.addPlayerFlags(builder, playerFlags);
  StateSnapshot.addWeaponHeatQ(builder, weaponHeatQ);
  StateSnapshot.addLoadoutBits(builder, loadoutBits);
  return StateSnapshot.endStateSnapshot(builder);
}

unpack(): StateSnapshotT {
  return new StateSnapshotT(
    this.serverTick(),
    this.lastProcessedInputSeq(),
    this.clientId(),
    this.posX(),
    this.posY(),
    this.posZ(),
    this.velX(),
    this.velY(),
    this.velZ(),
    this.weaponSlot(),
    this.ammoInMag(),
    this.dashCooldown(),
    this.health(),
    this.kills(),
    this.deaths(),
    this.viewYawQ(),
    this.viewPitchQ(),
    this.playerFlags(),
    this.weaponHeatQ(),
    this.loadoutBits()
  );
}


unpackTo(_o: StateSnapshotT): void {
  _o.serverTick = this.serverTick();
  _o.lastProcessedInputSeq = this.lastProcessedInputSeq();
  _o.clientId = this.clientId();
  _o.posX = this.posX();
  _o.posY = this.posY();
  _o.posZ = this.posZ();
  _o.velX = this.velX();
  _o.velY = this.velY();
  _o.velZ = this.velZ();
  _o.weaponSlot = this.weaponSlot();
  _o.ammoInMag = this.ammoInMag();
  _o.dashCooldown = this.dashCooldown();
  _o.health = this.health();
  _o.kills = this.kills();
  _o.deaths = this.deaths();
  _o.viewYawQ = this.viewYawQ();
  _o.viewPitchQ = this.viewPitchQ();
  _o.playerFlags = this.playerFlags();
  _o.weaponHeatQ = this.weaponHeatQ();
  _o.loadoutBits = this.loadoutBits();
}
}

export class StateSnapshotT implements flatbuffers.IGeneratedObject {
constructor(
  public serverTick: number = 0,
  public lastProcessedInputSeq: number = 0,
  public clientId: string|Uint8Array|null = null,
  public posX: number = 0.0,
  public posY: number = 0.0,
  public posZ: number = 0.0,
  public velX: number = 0.0,
  public velY: number = 0.0,
  public velZ: number = 0.0,
  public weaponSlot: number = 0,
  public ammoInMag: number = 0,
  public dashCooldown: number = 0.0,
  public health: number = 0.0,
  public kills: number = 0,
  public deaths: number = 0,
  public viewYawQ: number = 0,
  public viewPitchQ: number = 0,
  public playerFlags: number = 0,
  public weaponHeatQ: number = 0,
  public loadoutBits: number = 0
){}


pack(builder:flatbuffers.Builder): flatbuffers.Offset {
  const clientId = (this.clientId !== null ? builder.createString(this.clientId!) : 0);

  return StateSnapshot.createStateSnapshot(builder,
    this.serverTick,
    this.lastProcessedInputSeq,
    clientId,
    this.posX,
    this.posY,
    this.posZ,
    this.velX,
    this.velY,
    this.velZ,
    this.weaponSlot,
    this.ammoInMag,
    this.dashCooldown,
    this.health,
    this.kills,
    this.deaths,
    this.viewYawQ,
    this.viewPitchQ,
    this.playerFlags,
    this.weaponHeatQ,
    this.loadoutBits
  );
}
}
