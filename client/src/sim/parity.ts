...
const buildDefaultScript = (): PredictionInput[] => {
  ...
  for (let i = 0; i < 10; i += 1) {
    script.push({ moveX: 1, moveY: 0, sprint: false, jump: false, dash: false });
  }
  for (let i = 0; i < 5; i += 1) {
    script.push({ moveX: 1, moveY: 0, sprint: true, jump: false, dash: false });
  }
  for (let i = 0; i < 10; i += 1) {
    script.push({ moveX: 0, moveY: -1, sprint: false, jump: i === 0, dash: false });
  }
  return script;
};
